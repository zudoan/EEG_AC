...
from .model_doananhvu import train_models_doananhvu, train_swin_spectrogram_tl
from .model_nguyenhuutuanphat import train_models_nguyenhuutuanphat, train_cnn_spectrogram_grid, train_crnn_spectrogram_grid
from .model_lengoctien import train_models_lengoctien
from .model_phantrantuonvy import train_models_phantrantuonvy


"""Điểm vào (entrypoint) cho bước train.

Script này chạy toàn bộ pipeline theo thứ tự:
1) Lấy danh sách file train/test (theo glob trong config)
2) Thống kê nhãn nhanh + chẩn đoán rò rỉ dữ liệu (trùng basename/MD5)
3) Trích xuất feature: vector bandpower + ảnh spectrogram
4) Xử lý outlier cho bandpower bằng clipping theo quantile (fit trên train)
5) Train các mô hình và đánh giá trên tập test
6) Lưu artifact vào `saved_models/` để API/Web Dashboard sử dụng
"""



def _subsample(files, max_files):
    # Giới hạn số lượng file (phục vụ chạy nhanh/debug). Nếu None thì dùng hết.
...
def run():
    ...
    # Load danh sách file từ glob (đường dẫn đã được cấu hình trong src/config.py)
    ...
    # Subsample (nếu cấu hình) để giảm thời gian chạy
    ...
    # Lưu ý quan trọng: clip bounds chỉ được fit trên train, sau đó apply cho cả train/test
    ...
    # Chuẩn hoá vector bandpower để các mô hình cổ điển hoạt động ổn định hơn
    ...
    if RUN_TF_MODELS:
        ...
        crnn_info = train_crnn_spectrogram_grid(
            bundle.X_spec_train,
            bundle.y_train,
            bundle.X_spec_test,
            bundle.y_test,
        ...
        if crnn_info is not None:
            results['NguyenHuuTuanPhat - CRNN (Spectrogram)'] = crnn_info
    else:
        print('Skip TF models (CNN/CRNN). Set RUN_TF_MODELS=1 to enable.')

    if RUN_TORCH_MODELS:
        swin_info = train_swin_spectrogram_tl(
            bundle.X_spec_train,
            bundle.y_train,
            bundle.X_spec_test,
            bundle.y_test,
        ...
        if swin_info is not None:
            results['DoanAnhVu - Swin (Spectrogram)'] = swin_info
    else:
        print('Skip Torch models (Swin). Set RUN_TORCH_MODELS=1 to enable.')
