...
# Cấu hình chung cho pipeline phân loại EEG (SMNI_CMI)
RANDOM_STATE = 42
FS = 256
N_SAMPLES = 256
...
# Đường dẫn mặc định tới dữ liệu train/test.
# Lưu ý: dataset được gom vào thư mục `data/` cho gọn cấu trúc project.
DEFAULT_TRAIN_GLOB = 'data/SMNI_CMI_TRAIN/Train/*.csv'
DEFAULT_TEST_GLOB = 'data/SMNI_CMI_TEST/Test/*.csv'

REMOVE_TEST_DUPLICATES = True
MAX_FILES_TRAIN = None
MAX_FILES_TEST = None

# Nơi lưu model/scaler sau khi train
SAVE_DIR = 'saved_models'

GRIDSEARCH_CV_SPLITS = int(os.environ.get('GRIDSEARCH_CV_SPLITS', '2'))
RUN_TF_MODELS = os.environ.get('RUN_TF_MODELS', '0').strip() == '1'
RUN_TORCH_MODELS = os.environ.get('RUN_TORCH_MODELS', '0').strip() == '1'
TRAIN_VERBOSE = int(os.environ.get('TRAIN_VERBOSE', '1'))

# Tuỳ chọn làm sạch nhiễu EOG (nháy mắt) bằng ICA
